...
function ModelBadge({ name }) {
  const styles = {
    chatgpt:    { background: 'var(--green-bg, #e8f5ef)', color: 'var(--green, #1a6b45)'  },
    gemini:     { background: 'var(--danger-bg, #fee2e2)', color: 'var(--danger, #c0392b)'  },
...
function ScoreGauge({ score = 73, delta = 8 }) {
  ...
  const color         = score >= 70 ? 'var(--green, #1a6b45)' : score >= 50 ? '#f59e0b' : 'var(--danger, #c0392b)';
    ...
    <Card style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '28px 24px' }}>
      <div style={{ position: 'relative', width: 120, height: 120, marginBottom: 14 }}>
        <svg width="120" height="120" viewBox="0 0 120 120">
          <circle cx="60" cy="60" r="52" fill="none" stroke="var(--cream-2, #f0eeea)" strokeWidth="10"/>
      ...
      <div className="m-grid-1" style={{ display: 'grid', gridTemplateColumns: '260px 1fr', gap: 16, marginBottom: 18 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div className="m-grid-2" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
            ...
            <StatCard label="Times mentioned" value={run.total_mentions ?? 0}              sub={<span><span style={{ color: 'var(--green, #1a6b45)', fontWeight: 600 }}>{run.visibility_score ?? 0}%</span> mention rate</span>} />
            <StatCard label="Positive"        value={run.total_positive ?? 0}              accent="var(--green, #1a6b45)" valueColor="var(--green, #1a6b45)" sub="of all mentions" />
  ...
  return (
    <div style={{ padding:24, display:'flex', flexDirection:'column', gap:12 }}>
      {models.map(m => {
        ...
        const pc=pct>=70?'var(--green, #1a6b45)':pct>=50?'#f59e0b':'var(--danger, #c0392b)';
  ...
  return (
    <div style={{ padding: 24 }}>
      <div className="m-grid-2" style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10, marginBottom:18 }}>
        {[
          {label:'Mentions this scan',value:wowTotal,sub:'of '+rows.length+' queries',c:'var(--ink, #0a0a0a)'},
          {label:'vs last scan',value:(wowDelta>=0?'+':'')+wowDelta,sub:wowDelta>0?'improvement':wowDelta<0?'decline':'no change',c:wowDelta>0?'var(--green, #1a6b45)':wowDelta<0?'var(--danger, #c0392b)':'var(--taupe, #7a7670)'},
          {label:'New mentions',value:'+'+wowNew,sub:'gained this scan',c:'var(--green, #1a6b45)'},
      ...
      <Card style={{ overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 'var(--fs-sm, 0.8125rem)' }}>
          ...
          <tbody>
            {filtered.map((r, i) => (
              <tr key={i} style={{ borderBottom: '1px solid var(--cream, #f8f7f4)' }}>
                ...
                <td style={{ padding: '10px 16px', fontWeight: r.mention_position === 1 ? 700 : 400, color: r.mention_position === 1 ? 'var(--green, #1a6b45)' : 'var(--ink, #0a0a0a)' }}>
...
function CompetitorsTab({ report }) {
  ...
  const prio = { high: { bg:'var(--danger-bg, #fee2e2)', color:'var(--danger, #c0392b)', text:'High impact' }, medium: { bg:'#fef3c7', color:'var(--amber-tx, #92690a)', text:'Medium' }, low: { bg:'var(--cream-2, #f0eeea)', color:'var(--taupe, #7a7670)', text:'Low' } };
...
function QueriesTab() {
  ...
  if (loading) return <div style={{ padding: 40, textAlign: 'center', color: 'var(--taupe, #7a7670)' }}>Loading queries…</div>;
      ...
      <div className="m-grid-1" style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 16 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <Card style={{ overflow: 'hidden' }}>
            ...
            {/* Add query input */}
            {!locked && (
              <div style={{ padding: '12px 16px', borderTop: '1px solid var(--line, #e4e0d8)', display: 'flex', gap: 8 }}>
                <input

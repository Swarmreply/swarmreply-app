  ...
  async function loadData() {
    try {
      ...
      axios.get(`${API}/contacts`, { headers: authHeaders() })
        .then(r => { setSegments(r.data.segments || []); setContacts(r.data.contacts || []); })
        .catch(() => {});
  ...
  return (
    <DashboardLayout title="SMS Campaigns">
      ...
      limit={usage.limit ?? usage.sms_limit ?? 1000} resetAt={usage.resetAt} />
        ...
        {tab === 'contacts' && (
          <>
            <div style={{ fontWeight: 600, fontSize: '.95rem', marginBottom: 4 }}>Contacts with a mobile number</div>
            <div style={{ fontSize: '.84rem', color: '#7a7670', lineHeight: 1.6, marginBottom: 16 }}>
              Everyone in SwarmReply with a phone number on file — added via CSV, surveys, review requests, or webchat. These are the people reachable by SMS campaigns.
            </div>
            {contactsWithPhone.length === 0 ? (
              <Card style={{ padding: 28, textAlign: 'center', color: '#7a7670' }}>
                <div style={{ fontSize: '1.4rem', marginBottom: 8 }}>📱</div>
                <div style={{ fontWeight: 600, marginBottom: 4 }}>No contacts with a phone number yet</div>
                <div style={{ fontSize: '.84rem' }}>Import contacts with phone numbers in <strong>Grow › Bulk Send</strong>, or they'll be added automatically as customers come in.</div>
              </Card>
            ) : (
              <Card style={{ overflow: 'hidden' }}>
                <div style={{ padding: '14px 20px', borderBottom: '1px solid #e4e0d8', fontWeight: 600, fontSize: '.875rem' }}>
                  {contactsWithPhone.length} contact{contactsWithPhone.length !== 1 ? 's' : ''} with a mobile number
                </div>
                <div style={{ maxHeight: 'calc(100vh - 340px)', overflowY: 'auto' }}>
                  {contactsWithPhone.map(c => (
                    <div key={c.id} style={{ padding: '12px 20px', borderBottom: '1px solid #f8f7f4', display: 'grid', gridTemplateColumns: '1fr auto', gap: 16, alignItems: 'center' }}>
                      <div style={{ minWidth: 0 }}>
                        <div style={{ fontWeight: 600, fontSize: '.875rem', marginBottom: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{c.name || 'Unnamed contact'}</div>
                        <div style={{ fontSize: '.75rem', color: '#7a7670', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{c.phone}{c.email ? ` · ${c.email}` : ''}</div>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                        {c.segment && c.segment !== 'all' && <span style={{ fontSize: '.7rem', color: '#7a7670', background: '#f0eeea', padding: '3px 9px', borderRadius: 50, textTransform: 'capitalize' }}>{c.segment}</span>}
                        {c.opted_out && <span style={{ fontSize: '.68rem', fontWeight: 700, color: '#b03a2e', background: '#fbeae5', padding: '3px 9px', borderRadius: 50 }}>Opted out</span>}
                      </div>
                    </div>
                  ))}
                </div>

...
const label = { display: 'block', fontSize: 'var(--fs-xs, 0.75rem)', fontWeight: 700, color: 'var(--taupe, #7a7670)', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 6 };
const input = { width: '100%', padding: '10px 13px', border: '1.5px solid var(--line, #e4e0d8)', borderRadius: 'var(--r-xs, 8px)', fontSize: 'var(--fs-base, 0.875rem)', fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box', color: 'var(--tx, #1a1a18)' };
  ...
  return (
    <DashboardLayout>
      ...
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '8px 0 60px' }}>
        {flash && <div style={{ background: '#dcfce7', border: '1px solid #86efac', color: '#166534', borderRadius: 'var(--r-md, 16px)', padding: '11px 16px', marginBottom: 16, fontSize: 'var(--fs-base, 0.875rem)', fontWeight: 700, display: 'flex', alignItems: 'center', gap: 8 }}><span>{'\u2713'}</span>{flash}</div>}
        {err && <div style={{ background: 'var(--danger-bg, #fee2e2)', border: '1px solid #fca5a5', color: 'var(--danger, #c0392b)', borderRadius: 'var(--r-md, 16px)', padding: '12px 16px', marginBottom: 16, fontSize: 'var(--fs-base, 0.875rem)', fontWeight: 600 }}>{err}</div>}
        {loading ? (
          ...
          return (
            <>
              ...
              {filtered.length === 0 ? (
                <Card style={{ textAlign: 'center', color: 'var(--taupe, #7a7670)', padding: 32, fontSize: 'var(--fs-base, 0.875rem)' }}>No surveys match {'\u201C'}{q}{'\u201D'}.</Card>
              ) : (
    ...
    <Card style={{ marginBottom: 12, display: 'flex', alignItems: 'center', gap: 14 }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          <span style={{ fontWeight: 700, color: 'var(--tx, #1a1a18)', fontSize: 'var(--fs-lg, 1rem)' }}>{t.name || 'Untitled survey'}</span>
          <span style={{ fontSize: 'var(--fs-2xs, 0.6875rem)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.05em', color: tc.fg, background: tc.bg, borderRadius: 'var(--r-pill, 999px)', padding: '2px 8px' }}>{isCustom ? 'Custom' : 'NPS'}</span>
          {t.is_default && <span style={{ fontSize: 'var(--fs-2xs, 0.6875rem)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.05em', color: 'var(--amber-tx, #92690a)', background: '#fef9c3', borderRadius: 'var(--r-pill, 999px)', padding: '2px 8px' }}>Default</span>}
          {t.scope === 'location' && t.location_id && <span style={{ fontSize: 'var(--fs-2xs, 0.6875rem)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.05em', color: '#3730a3', background: '#eef2ff', borderRadius: 'var(--r-pill, 999px)', padding: '2px 8px' }}>{locName ? locName(t.location_id) : 'Location'}</span>}
        </div>
        <div style={{ fontSize: 'var(--fs-xs, 0.75rem)', color: 'var(--mute, #a8a39a)', marginTop: 3 }}>
      ...
      {!t.is_default && <button onClick={onDelete} title="Delete survey" style={{ width: 34, height: 34, borderRadius: 'var(--r-xs, 8px)', border: '1.5px solid #f0d0d0', background: 'white', color: 'var(--danger, #c0392b)', cursor: 'pointer', fontSize: 'var(--fs-lg, 1rem)', fontWeight: 700, fontFamily: 'inherit' }}>{'\u00D7'}</button>}
    </Card>
  );
}
...
function TypeCard({ title, desc, foot, tone, bg, onClick }) {
  return (
    <button onClick={onClick} style={{ textAlign: 'left', background: 'white', border: '1.5px solid var(--line, #e4e0d8)', borderRadius: 'var(--r-md, 16px)', padding: 22, cursor: 'pointer', fontFamily: 'inherit', display: 'flex', flexDirection: 'column', gap: 10, transition: 'border-color .15s' }}
      onMouseEnter={(e) => { e.currentTarget.style.borderColor = tone; }} onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'var(--line, #e4e0d8)'; }}>
      <span style={{ alignSelf: 'flex-start', fontSize: 'var(--fs-2xs, 0.6875rem)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.06em', color: tone, background: bg, borderRadius: 'var(--r-pill, 999px)', padding: '3px 10px' }}>{title}</span>
      <p style={{ fontSize: 'var(--fs-base, 0.875rem)', color: 'var(--tx-2, #4a4a48)', margin: 0, lineHeight: 1.6 }}>{desc}</p>
      <span style={{ fontSize: 'var(--fs-xs, 0.75rem)', color: 'var(--mute, #a8a39a)', marginTop: 'auto' }}>{foot}</span>
    </button>
  );
}
...
function Toggle({ on, onChange }) {
  return (
    <button onClick={() => onChange(!on)} aria-pressed={on} style={{ width: 46, height: 27, borderRadius: 'var(--r-pill, 999px)', border: 'none', cursor: 'pointer', background: on ? 'var(--green, #1a6b45)' : '#d4cfc5', position: 'relative', flexShrink: 0, transition: 'background .15s', padding: 0 }}>
      <span style={{ position: 'absolute', top: 3, left: on ? 22 : 3, width: 21, height: 21, borderRadius: 'var(--r-full, 50%)', background: 'white', transition: 'left .15s', boxShadow: '0 1px 2px rgba(0,0,0,.2)' }} />
    </button>
  );
}
...
function ChannelChip({ active, disabled, label, sub, onClick }) {
  return (
    <button onClick={disabled ? undefined : onClick} disabled={disabled} style={{
      flex: 1, padding: '13px 16px', borderRadius: 'var(--r-md, 16px)', border: '1.5px solid',
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <span style={{ fontWeight: 700, fontSize: 'var(--fs-base, 0.875rem)', color: 'var(--tx, #1a1a18)' }}>{label}</span>
        <span style={{ width: 18, height: 18, borderRadius: 'var(--r-full, 50%)', border: '2px solid', borderColor: active ? 'var(--honey, #f5c842)' : '#d4cfc5', background: active ? 'var(--honey, #f5c842)' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{active && <span style={{ width: 7, height: 7, borderRadius: 'var(--r-full, 50%)', background: '#1a1408' }} />}</span>
      </div>
      <div style={{ fontSize: 'var(--fs-xs, 0.75rem)', color: active ? '#7a5a06' : 'var(--mute, #a8a39a)', marginTop: 4, fontWeight: 600 }}>{sub}</div>
    </button>
  );
}

function ContactPicker({ contacts, selected, setSelected, search, setSearch }) {
  const linkBtn = { border: 'none', background: 'none', color: 'var(--taupe, #7a7670)', fontWeight: 700, fontSize: 'var(--fs-xs, 0.75rem)', fontFamily: 'inherit', cursor: 'pointer', padding: 0 };
  ...
  return (
    <div style={{ marginTop: 14 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '10px 2px 8px' }}>
        <span style={{ fontSize: 'var(--fs-xs, 0.75rem)', color: 'var(--taupe, #7a7670)', fontWeight: 600 }}>{selected.length} selected{contacts.length ? ` of ${contacts.length}` : ''}</span>
        <span>
      ...
      {contacts.length === 0 ? (
        <div style={{ fontSize: 'var(--fs-sm, 0.8125rem)', color: 'var(--mute, #a8a39a)', padding: '14px 2px' }}>No contacts with an email yet. Add some in <a href="/dashboard/grow?tab=import" style={{ color: 'var(--taupe, #7a7670)', fontWeight: 600 }}>Grow › Import</a>.</div>
      ) : (
        <div style={{ maxHeight: 280, overflowY: 'auto', border: '1px solid var(--cream-2, #f0eeea)', borderRadius: 'var(--r-sm, 10px)' }}>
          {filtered.length === 0 && <div style={{ fontSize: 'var(--fs-sm, 0.8125rem)', color: 'var(--mute, #a8a39a)', padding: 16 }}>No matches.</div>}
          {filtered.map((c) => {
            ...
            return (
              <div key={c.id || c.email} onClick={disabled ? undefined : () => toggle(c.email)} style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '9px 13px', borderTop: '1px solid #f5f3ef', cursor: disabled ? 'default' : 'pointer', opacity: disabled ? .5 : 1 }}>
                ...
                <div style={{ minWidth: 0, flex: 1 }}>
                  ...
                  {c.name && <div style={{ fontSize: 'var(--fs-xs, 0.75rem)', color: 'var(--mute, #a8a39a)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.email}</div>}
                </div>
                {disabled && <span style={{ fontSize: 'var(--fs-2xs, 0.6875rem)', fontWeight: 700, color: 'var(--mute, #a8a39a)', flexShrink: 0, letterSpacing: '.04em' }}>OPTED OUT</span>}
              </div>
            );
          })}
        </div>
      ...
      <div style={{ maxWidth: 620, margin: '0 auto', padding: '8px 0 60px' }}>
        {result ? (
          <Card style={{ padding: 28, textAlign: 'center' }}>
            <div style={{ fontSize: 'var(--fs-3xl, 2rem)', marginBottom: 8 }}>{result.scheduled ? '\u23F0' : '\u2713'}</div>
            <div style={{ fontWeight: 700, fontSize: 'var(--fs-lg, 1rem)', color: 'var(--tx, #1a1a18)', marginBottom: 8 }}>{result.scheduled ? 'Survey scheduled' : 'Survey sent'}</div>
            <p style={{ fontSize: 'var(--fs-base, 0.875rem)', color: 'var(--tx-2, #4a4a48)', lineHeight: 1.6, margin: '0 0 20px' }}>
              {result.scheduled
          ...
          <>
            <Card style={{ padding: 24, marginBottom: 16 }}>
              ...
              {!loading && surveys.length === 0 && <p style={{ fontSize: 'var(--fs-sm, 0.8125rem)', color: 'var(--mute, #a8a39a)', margin: '10px 0 0' }}>Create a survey first, then come back to send it.</p>}
            </Card>
            ...
            <Card style={{ padding: 24, marginBottom: 16 }}>
              ...
              {audienceMode === 'segment' ? (
                <>
                  ...
                  <p style={{ fontSize: 'var(--fs-xs, 0.75rem)', color: 'var(--mute, #a8a39a)', margin: '10px 0 0' }}>Contacts are managed in <a href="/dashboard/grow?tab=import" style={{ color: 'var(--taupe, #7a7670)', fontWeight: 600 }}>Grow › Import</a>. Anyone opted out is skipped automatically.</p>
                </>
              ) : (
            ...
            <Card style={{ padding: 24, marginBottom: 16 }}>
              ...
              <p style={{ fontSize: 'var(--fs-xs, 0.75rem)', color: 'var(--mute, #a8a39a)', margin: '12px 0 0' }}>Text (SMS) unlocks once your carrier registration is approved.</p>
            </Card>

            <Card style={{ padding: 24, marginBottom: 16 }}>
              <SectionLabel>Email preview</SectionLabel>
              <div style={{ marginTop: 10, border: '1.5px solid var(--line, #e4e0d8)', borderRadius: 'var(--r-md, 16px)', overflow: 'hidden', fontFamily: 'Arial, sans-serif' }}>
                <div style={{ background: brandColor, padding: '20px 32px' }}>
                  {brand.brandLogo
                    ...
                    : <div style={{ fontWeight: 800, fontSize: 'var(--fs-lg, 1rem)', color: 'var(--ink, #0a0a0a)' }}>{bizName}</div>}
                </div>
                <div style={{ padding: 32, background: 'white' }}>
                  <h2 style={{ margin: '0 0 16px', fontSize: 'var(--fs-xl, 1.25rem)', color: 'var(--ink, #0a0a0a)' }}>How was your experience, {sampleName}?</h2>
                  <div style={{ fontSize: 'var(--fs-base, 0.875rem)', lineHeight: 1.75, color: 'var(--tx-3, #3a3a38)', marginBottom: 26 }}>We'd love your honest feedback about your experience with {bizName}. It takes about 30 seconds and helps us do better.</div>
                  <div style={{ textAlign: 'center' }}><span style={{ display: 'inline-block', background: brandColor, color: 'var(--ink, #0a0a0a)', padding: '14px 30px', borderRadius: 'var(--r-pill, 999px)', fontWeight: 700, fontSize: 'var(--fs-base, 0.875rem)' }}>Start the survey →</span></div>
                </div>
                <div style={{ background: brandColor, padding: '14px 32px', textAlign: 'center' }}>
                  <span style={{ fontSize: 'var(--fs-xs, 0.75rem)', color: 'var(--ink, #0a0a0a)', opacity: .65 }}>Sent by {bizName} via SwarmReply</span>
                </div>
              </div>
              <p style={{ fontSize: 'var(--fs-xs, 0.75rem)', color: 'var(--mute, #a8a39a)', margin: '10px 0 0' }}>Your logo and brand color come from <a href="/dashboard/grow?tab=templates" style={{ color: 'var(--taupe, #7a7670)', fontWeight: 600 }}>Grow › Request Templates</a>.</p>
            </Card>

            <Card style={{ padding: 24, marginBottom: 16, opacity: .9 }}>
              <SectionLabel>Text preview</SectionLabel>
              <div style={{ marginTop: 10 }}>
                <div style={{ background: '#e7ebf0', color: 'var(--tx, #1a1a18)', padding: '11px 15px', borderRadius: 'var(--r-md, 16px)', borderBottomLeftRadius: 4, fontSize: 'var(--fs-base, 0.875rem)', lineHeight: 1.5, display: 'inline-block', maxWidth: 340 }}>
                  Hi {sampleName}, {bizName} would love your quick feedback — it takes 30 seconds: app.swarmreply.com/r/…
                </div>
              </div>
              <p style={{ fontSize: 'var(--fs-xs, 0.75rem)', color: 'var(--mute, #a8a39a)', margin: '12px 0 0' }}>Preview only — texting turns on once SMS is approved.</p>
            </Card>
                ...
                <>
                  <div style={{ display: 'flex', gap: 10, marginTop: 14, flexWrap: 'wrap' }}>
                    <div style={{ flex: 1, minWidth: 150 }}>
                      <label style={{ fontSize: 'var(--fs-xs, 0.75rem)', fontWeight: 700, color: 'var(--taupe, #7a7670)', display: 'block', marginBottom: 4 }}>Date</label>
                    ...
                    <div style={{ flex: 1, minWidth: 120 }}>
                      <label style={{ fontSize: 'var(--fs-xs, 0.75rem)', fontWeight: 700, color: 'var(--taupe, #7a7670)', display: 'block', marginBottom: 4 }}>Time</label>
                      <input type="time" value={schedTime} onChange={(e) => setSchedTime(e.target.value)} style={{ ...input, background: 'white' }} />
                    </div>
                  </div>
                  <p style={{ fontSize: 'var(--fs-xs, 0.75rem)', color: 'var(--mute, #a8a39a)', margin: '12px 0 0' }}>Uses this device's clock. It'll appear under Scheduled sends below until it goes out — you can cancel it any time before then.</p>
                </>
              )}
            </Card>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              ...
              {!channels.email && <span style={{ fontSize: 'var(--fs-sm, 0.8125rem)', color: 'var(--mute, #a8a39a)' }}>Turn on a channel to send.</span>}
            </div>
          </>
        )}
        ...
        {scheduled.length > 0 && (
          <Card style={{ padding: 24, marginTop: 8 }}>
            ...
            <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 8 }}>
              {scheduled.map((s) => (
                <div key={s.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, border: '1px solid var(--cream-2, #f0eeea)', borderRadius: 'var(--r-sm, 10px)', padding: '11px 14px' }}>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontWeight: 600, fontSize: 'var(--fs-base, 0.875rem)', color: 'var(--tx, #1a1a18)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.survey_name || 'Default survey'} <span style={{ color: 'var(--mute, #a8a39a)', fontWeight: 500 }}>{'\u2192'} {Array.isArray(s.contact_emails) && s.contact_emails.length === 1 ? s.contact_emails[0] : (Array.isArray(s.contact_emails) && s.contact_emails.length ? `${s.contact_emails.length} people` : (s.segment === 'all' ? 'All contacts' : s.segment))}</span></div>
                    <div style={{ fontSize: 'var(--fs-xs, 0.75rem)', color: 'var(--taupe, #7a7670)', marginTop: 2, display: 'flex', alignItems: 'center', gap: 7, flexWrap: 'wrap' }}>
                      <span>{new Date(s.send_at).toLocaleString([], { dateStyle: 'medium', timeStyle: 'short' })}</span>
                      {s.source && s.source !== 'manual' && (
                        <span style={{ fontSize: 'var(--fs-2xs, 0.6875rem)', fontWeight: 700, background: '#fff6dc', color: '#8a6d1a', padding: '1px 8px', borderRadius: 'var(--r-pill, 999px)', whiteSpace: 'nowrap' }}>
                          {s.source === 'integration' ? 'Automated · Integration' : s.source === 'csv_import' ? 'Automated · Visit date' : 'Automated'}
                        </span>
                      )}
                    </div>
                  </div>
                  <button onClick={() => cancelScheduled(s.id)} style={{ flexShrink: 0, padding: '7px 13px', borderRadius: 'var(--r-xs, 8px)', border: '1.5px solid var(--line, #e4e0d8)', background: 'white', color: 'var(--taupe, #7a7670)', fontSize: 'var(--fs-xs, 0.75rem)', fontWeight: 700, fontFamily: 'inherit', cursor: 'pointer' }}>Cancel</button>
                </div>
              ))}
            </div>
          </Card>
        ...
        {(
          <>
            ...
            <Card style={{ marginBottom: 16 }}>
              ...
              <p style={{ fontSize: 'var(--fs-xs, 0.75rem)', color: 'var(--mute, #a8a39a)', margin: '8px 0 0' }}>Just for you — customers never see this.</p>
              {locations.length > 1 && (
                <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid var(--cream-2, #f0eeea)' }}>
                  ...
                  <p style={{ fontSize: 'var(--fs-xs, 0.75rem)', color: 'var(--mute, #a8a39a)', margin: '8px 0 0', lineHeight: 1.5 }}>
            ...
            <Card style={{ marginBottom: 16 }}>
              ...
              <p style={{ fontSize: 'var(--fs-sm, 0.8125rem)', color: 'var(--taupe, #7a7670)', margin: '4px 0 16px', lineHeight: 1.55 }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 18, flexWrap: 'wrap' }}>
                {SCALE_OPTIONS.map((o) => {
                  ...
                  return (
                    <button key={o.type} onClick={() => setScale(o.type)} style={{ padding: '9px 16px', borderRadius: 'var(--r-pill, 999px)', border: '1.5px solid', borderColor: on ? 'var(--honey, #f5c842)' : 'var(--line, #e4e0d8)', background: on ? '#fffbe9' : 'white', fontWeight: 700, fontSize: 'var(--fs-sm, 0.8125rem)', color: on ? '#7a5a06' : 'var(--taupe, #7a7670)', cursor: 'pointer', fontFamily: 'inherit' }}>{o.label}</button>
                  );
                })}
              </div>
              ...
              <div style={{ marginTop: 22 }}>
                ...
                <div style={{ display: 'flex', height: 38, borderRadius: 'var(--r-sm, 10px)', overflow: 'hidden', border: '1px solid var(--line, #e4e0d8)', marginBottom: 14 }}>
            ...
            {cfg.type === 'custom' && (<>
              <Card style={{ marginBottom: 16 }}>
                ...
                <p style={{ fontSize: 'var(--fs-sm, 0.8125rem)', color: 'var(--taupe, #7a7670)', margin: '4px 0 12px', lineHeight: 1.55 }}>An optional welcome shown before the first question.</p>
              ...
              <PathEditor
                path={{ key: 'questions', label: 'Questions', desc: 'Asked in order', tone: '#6d28d9', bg: '#ede9fe', bd: '#ddd6fe' }}
              ...
              <Card style={{ marginBottom: 16, display: 'flex', alignItems: 'flex-start', gap: 14 }}>
                <div style={{ flex: 1 }}>
                  ...
                  <p style={{ fontSize: 'var(--fs-sm, 0.8125rem)', color: 'var(--taupe, #7a7670)', margin: '4px 0 0', lineHeight: 1.55 }}>
...
function Band({ tone, bg, text, flex }) {
  return <div style={{ flex: Math.max(0.5, flex), background: bg, color: tone, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'var(--fs-2xs, 0.6875rem)', fontWeight: 700, padding: '0 4px', textAlign: 'center', whiteSpace: 'nowrap', overflow: 'hidden' }}>{text}</div>;
}

function Stepper({ title, value, min, max, onChange }) {
  const btn = { width: 32, height: 32, borderRadius: 'var(--r-xs, 8px)', border: '1.5px solid var(--line, #e4e0d8)', background: 'white', cursor: 'pointer', fontSize: 'var(--fs-lg, 1rem)', fontWeight: 700, color: 'var(--taupe, #7a7670)', lineHeight: 1, fontFamily: 'inherit' };
  return (
    <div>
      <div style={{ fontSize: 'var(--fs-xs, 0.75rem)', color: 'var(--taupe, #7a7670)', fontWeight: 600, marginBottom: 6 }}>{title}</div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <button style={btn} onClick={() => onChange(Math.max(min, value - 1))} disabled={value <= min}>{'\u2212'}</button>
        <span style={{ minWidth: 26, textAlign: 'center', fontWeight: 800, fontSize: 'var(--fs-lg, 1rem)', color: 'var(--tx, #1a1a18)' }}>{value}</span>
...
function PathEditor({ path, blocks, message, onMessage, showMessage, onAdd, onUpdate, onRemove, onMove, allowConditions }) {
  const [adding, setAdding] = useState(false);
  return (
    <Card style={{ marginBottom: 16, borderColor: path.bd }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
        <span style={{ fontSize: 'var(--fs-2xs, 0.6875rem)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.05em', color: path.tone, background: path.bg, border: `1px solid ${path.bd}`, borderRadius: 'var(--r-pill, 999px)', padding: '4px 11px' }}>{path.label}</span>
        <span style={{ fontSize: 'var(--fs-sm, 0.8125rem)', color: 'var(--mute, #a8a39a)' }}>{path.desc}</span>
      </div>
      ...
      {blocks.length === 0 ? (
        <p style={{ fontSize: 'var(--fs-sm, 0.8125rem)', color: 'var(--mute, #a8a39a)', margin: '0 0 12px', fontStyle: 'italic' }}>No follow-up questions — this group goes straight to the public review invite.</p>
      ) : (
        blocks.map((b, i) => (
      ...
      {adding ? (
        <div style={{ border: '1.5px dashed var(--line, #e4e0d8)', borderRadius: 'var(--r-md, 16px)', padding: 12, marginTop: 6 }}>
          <div style={{ fontSize: 'var(--fs-xs, 0.75rem)', fontWeight: 700, color: 'var(--taupe, #7a7670)', marginBottom: 10 }}>Choose a question type</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(150px,1fr))', gap: 8 }}>
            {BLOCK_TYPES.filter((t) => allowConditions || t.type !== 'end').map((t) => (
              <button key={t.type} onClick={() => { onAdd(t.type); setAdding(false); }} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 12px', borderRadius: 'var(--r-sm, 10px)', border: '1.5px solid var(--line, #e4e0d8)', background: 'white', cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left' }}>
                <span style={{ fontSize: 'var(--fs-lg, 1rem)' }}>{t.icon}</span>
                <span><span style={{ display: 'block', fontWeight: 700, fontSize: 'var(--fs-sm, 0.8125rem)', color: 'var(--tx, #1a1a18)' }}>{t.label}</span><span style={{ fontSize: 'var(--fs-2xs, 0.6875rem)', color: 'var(--mute, #a8a39a)' }}>{t.hint}</span></span>
              </button>
            ))}
          </div>
          <button onClick={() => setAdding(false)} style={{ marginTop: 10, background: 'none', border: 'none', color: 'var(--mute, #a8a39a)', fontSize: 'var(--fs-xs, 0.75rem)', cursor: 'pointer', fontFamily: 'inherit' }}>Cancel</button>
        </div>
      ) : (
        <button onClick={() => setAdding(true)} style={{ width: '100%', padding: 11, borderRadius: 'var(--r-sm, 10px)', border: '1.5px dashed #d8d4cc', background: 'transparent', color: 'var(--taupe, #7a7670)', fontWeight: 700, fontSize: 'var(--fs-sm, 0.8125rem)', cursor: 'pointer', fontFamily: 'inherit', marginTop: 6 }}>+ Add a question</button>
      )}
    </Card>
  );
}
...
function BlockCard({ block, idx, total, priorSourceBlocks, onUpdate, onRemove, onMove }) {
  ...
  const mini = { width: 26, height: 26, borderRadius: 'var(--r-xs, 8px)', border: '1.5px solid var(--line, #e4e0d8)', background: 'white', cursor: 'pointer', color: 'var(--mute, #a8a39a)', fontSize: 'var(--fs-base, 0.875rem)', lineHeight: 1, fontFamily: 'inherit' };

  return (
    <div style={{ border: '1px solid #ece9e3', borderRadius: 'var(--r-md, 16px)', padding: 14, marginBottom: 10, background: '#fcfbf9' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
        <span style={{ fontSize: 'var(--fs-lg, 1rem)' }}>{meta.icon}</span>
        <span style={{ fontSize: 'var(--fs-2xs, 0.6875rem)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.05em', color: 'var(--taupe, #7a7670)' }}>{meta.label}</span>
      ...
      {isEnd ? (
        <p style={{ fontSize: 'var(--fs-sm, 0.8125rem)', color: 'var(--taupe, #7a7670)', margin: '0 0 2px', lineHeight: 1.55 }}>The survey ends here for customers who meet the condition below — they go straight to the review invite. Everyone else keeps going.</p>
      ) : (
      ...
      {hasOptions && (
        <div style={{ marginTop: 10 }}>
          <div style={{ fontSize: 'var(--fs-2xs, 0.6875rem)', fontWeight: 700, color: 'var(--mute, #a8a39a)', marginBottom: 6 }}>Options</div>
          ...
          <button onClick={addOpt} style={{ background: 'none', border: 'none', color: '#7a5a06', fontSize: 'var(--fs-xs, 0.75rem)', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>+ Add option</button>
          {isMC && (
            <label style={{ display: 'flex', alignItems: 'center', gap: 7, marginTop: 8, fontSize: 'var(--fs-xs, 0.75rem)', color: 'var(--taupe, #7a7670)', cursor: 'pointer' }}>
      ...
      {isContact && (
        <div style={{ marginTop: 10 }}>
          <div style={{ fontSize: 'var(--fs-2xs, 0.6875rem)', fontWeight: 700, color: 'var(--mute, #a8a39a)', marginBottom: 6 }}>Fields to collect</div>
          {[['name', 'Name'], ['email', 'Email'], ['phone', 'Phone']].map(([k, lbl]) => {
            const fields = block.fields || ['name', 'email', 'phone'];
            return (
              <label key={k} style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 5, fontSize: 'var(--fs-sm, 0.8125rem)', color: 'var(--taupe, #7a7670)', cursor: 'pointer' }}>
...
const condSelStyle = { border: '1.5px solid var(--line, #e4e0d8)', borderRadius: 'var(--r-xs, 8px)', padding: '6px 9px', fontSize: 'var(--fs-xs, 0.75rem)', fontFamily: 'inherit', background: 'white', color: 'var(--tx, #1a1a18)', maxWidth: 200 };
...
function ConditionEditor({ condition, priorBlocks, onChange, forEnd }) {
  ...
  if (!priorBlocks.length) {
    return forEnd
      ? <p style={{ fontSize: 'var(--fs-xs, 0.75rem)', color: 'var(--mute, #a8a39a)', margin: '8px 0 0' }}>Add a choice or rating question before this block to choose when the survey ends.</p>
      : null;
  }
    ...
    return (
      <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px dashed #e9e5dd' }}>
        <button onClick={() => onChange({ match: 'all', rules: [defaultRule(priorBlocks[0])] })}
          style={{ background: 'none', border: 'none', color: '#6d28d9', fontSize: 'var(--fs-xs, 0.75rem)', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', padding: 0 }}>
  ...
  return (
    <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px dashed #e9e5dd' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8, flexWrap: 'wrap' }}>
        <span style={{ fontSize: 'var(--fs-2xs, 0.6875rem)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.05em', color: '#6d28d9' }}>{forEnd ? 'End the survey if' : 'Show only if'}</span>
        {rules.length > 1 && (
      ...
      <div style={{ display: 'flex', gap: 14, marginTop: 4 }}>
        <button onClick={addRule} style={{ background: 'none', border: 'none', color: '#6d28d9', fontSize: 'var(--fs-xs, 0.75rem)', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', padding: 0 }}>+ Add condition</button>
        <button onClick={() => onChange(null)} style={{ background: 'none', border: 'none', color: 'var(--mute, #a8a39a)', fontSize: 'var(--fs-xs, 0.75rem)', cursor: 'pointer', fontFamily: 'inherit', padding: 0 }}>Remove</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ marginBottom: 8 }}>
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center', marginBottom: 6 }}>
        ...
        <button onClick={onRemove} title="Remove condition" style={{ width: 24, height: 24, borderRadius: 'var(--r-xs, 8px)', border: '1.5px solid var(--line, #e4e0d8)', background: 'white', color: 'var(--mute, #a8a39a)', cursor: 'pointer', fontSize: 'var(--fs-sm, 0.8125rem)', fontFamily: 'inherit', lineHeight: 1 }}>{'\u00D7'}</button>
      </div>
      {!isNumeric && (
        options.length > 0 ? (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            {options.map((o) => {
              ...
              return <button key={o} onClick={() => toggle(o)} style={{ padding: '4px 10px', borderRadius: 'var(--r-pill, 999px)', border: '1.5px solid', borderColor: on ? '#6d28d9' : 'var(--line, #e4e0d8)', background: on ? '#f5f3ff' : 'white', color: on ? '#6d28d9' : 'var(--taupe, #7a7670)', fontSize: 'var(--fs-xs, 0.75rem)', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>{on ? '\u2713 ' : ''}{o}</button>;
            })}
          </div>
        ) : (
          <p style={{ fontSize: 'var(--fs-xs, 0.75rem)', color: 'var(--mute, #a8a39a)', margin: 0 }}>Add options to that question to choose values.</p>
        )
      )}
    </div>
  );

...
function SendSurvey({ onBack }) {
  ...
  const [brand, setBrand] = useState({ brandColor: '#f5c842', brandLogo: '', brandLogoPosition: 'left' });
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    Promise.all([
      ...
      setBrand({ brandColor: t.brandColor || '#f5c842', brandLogo: t.brandLogo || '', brandLogoPosition: t.brandLogoPosition || 'left' });
          ...
          <>
            ...
            <Card style={{ padding: 24, marginBottom: 16 }}>
              ...
              <div style={{ marginTop: 10, border: '1.5px solid #e4e0d8', borderRadius: 12, overflow: 'hidden', fontFamily: 'Arial, sans-serif' }}>
                <div style={{ background: brandColor, padding: '20px 32px', display: 'flex', alignItems: 'center', justifyContent: ({ left: 'flex-start', middle: 'center', right: 'flex-end' })[brand.brandLogoPosition] || 'flex-start' }}>
                  {brand.brandLogo
                    ? <img src={brand.brandLogo} alt={bizName} style={{ maxHeight: 48, maxWidth: 160, objectFit: 'contain' }} onError={e => e.target.style.display='none'} />

...
function StatusBadge({ status }) {
  ...
  return (
    <span style={{ background: bg, color, fontSize: 'var(--fs-2xs, 0.6875rem)', fontWeight: 700,
      padding: '2px 9px', borderRadius: 'var(--r-pill, 999px)' }}>
      {label}
    </span>
  );
}
...
function SendTiming({ provider, currentMinutes, currentType, currentSurveyId, onSaved }) {
  ...
  const inputStyle = {
    border: '1.5px solid var(--line, #e4e0d8)', borderRadius: 'var(--r-xs, 8px)', padding: '7px 10px',
    fontSize: 'var(--fs-sm, 0.8125rem)', fontFamily: 'inherit', background: 'white', color: 'var(--tx, #1a1a18)',
  };
  ...
  return (
    <div style={{
      ...
      <div style={rowStyle}>
        <span style={{ fontSize: 'var(--fs-xs, 0.75rem)', fontWeight: 700, color: 'var(--tx, #1a1a18)' }}>Send:</span>
        ...
        {followUpType === 'survey' && <span style={{ fontSize: 'var(--fs-xs, 0.75rem)', color: 'var(--mute, #a8a39a)' }}>respondents are also invited to leave a review</span>}
      </div>
      <div style={rowStyle}>
        <span style={{ fontSize: 'var(--fs-xs, 0.75rem)', fontWeight: 700, color: 'var(--tx, #1a1a18)' }}>Send timing:</span>
        ...
        <button onClick={save} disabled={saving} style={{
          marginLeft: 'auto', padding: '7px 16px', borderRadius: 'var(--r-pill, 999px)', border: 'none',
          background: saved ? 'var(--green, #1a6b45)' : 'var(--ink, #0a0a0a)', color: 'white', cursor: 'pointer',
          fontSize: 'var(--fs-xs, 0.75rem)', fontWeight: 700, fontFamily: 'inherit',
    ...
    <div className="sr-card" style={{
      ...
      <div style={{ padding: '18px 20px', display: 'flex', alignItems: 'center', gap: 14 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3 }}>
            <span style={{ fontWeight: 700, fontSize: 'var(--fs-base, 0.875rem)' }}>{integration.name}</span>
            <StatusBadge status={connectedData?.status} />
          </div>
          <div style={{ fontSize: 'var(--fs-xs, 0.75rem)', color: 'var(--taupe, #7a7670)', lineHeight: 1.5 }}>
            {integration.description}
          </div>
          {connected && (
            <div style={{ fontSize: 'var(--fs-xs, 0.75rem)', color: 'var(--green, #1a6b45)', marginTop: 4, display: 'flex', gap: 12 }}>
          ...
          {connected ? (
            <>
              <button onClick={handleDisconnect} style={{
                padding: '7px 14px', borderRadius: 'var(--r-pill, 999px)', border: '1.5px solid var(--line, #e4e0d8)',
                background: 'transparent', cursor: 'pointer', fontSize: 'var(--fs-sm, 0.8125rem)',
                fontWeight: 600, color: 'var(--taupe, #7a7670)', fontFamily: 'inherit',
              }}>Disconnect</button>
            </>
          ) : (
            <button onClick={() => setExpanded(e => !e)} style={{
              padding: '8px 18px', borderRadius: 'var(--r-pill, 999px)',
              background: 'var(--ink, #0a0a0a)', color: 'white', border: 'none',
              cursor: 'pointer', fontSize: 'var(--fs-sm, 0.8125rem)', fontWeight: 700, fontFamily: 'inherit',
            }}>
      ...
      {expanded && !connected && (
        <div style={{ padding: '0 20px 20px', borderTop: '1px solid var(--cream-2, #f0eeea)' }}>
          {error && (
            <div style={{ background: 'var(--danger-bg, #fee2e2)', border: '1px solid #fecaca', borderRadius: 'var(--r-xs, 8px)',
              padding: '9px 13px', fontSize: 'var(--fs-sm, 0.8125rem)', color: 'var(--danger, #c0392b)', margin: '12px 0' }}>
              {error}
            </div>
          )}
                ...
                <div style={{ marginTop: 12 }}>
                  ...
                  <input
                    ...
                    style={{ width: '100%', padding: '9px 12px', border: '1.5px solid var(--line, #e4e0d8)',
                      borderRadius: 'var(--r-xs, 8px)', fontSize: 'var(--fs-lg, 1rem)', fontFamily: 'inherit', outline: 'none' }}
                  />
                </div>
              )}
              <p style={{ fontSize: 'var(--fs-xs, 0.75rem)', color: 'var(--taupe, #7a7670)', margin: '12px 0', lineHeight: 1.6 }}>
          ...
          <div style={{ marginTop: 14, display: 'flex', gap: 9, alignItems: 'center' }}>
            <button onClick={handleConnect} disabled={connecting} style={{
              padding: '10px 22px', borderRadius: 'var(--r-pill, 999px)', background: 'var(--ink, #0a0a0a)',
              color: 'white', border: 'none', cursor: 'pointer', fontSize: 'var(--fs-base, 0.875rem)',
...
export default function Integrations() {
  ...
  const selectStyle = {
    padding: '10px 14px', borderRadius: 'var(--r-sm, 10px)', border: '1.5px solid var(--line, #e4e0d8)',
    background: 'white', fontSize: 'var(--fs-sm, 0.8125rem)', fontFamily: 'inherit',
        ...
        <div style={{ display: 'flex', gap: 2, borderBottom: '1px solid var(--line, #e4e0d8)', marginBottom: 20 }}>
          {[{ id: 'integrations', label: 'Integrations' }, { id: 'zapier', label: 'Zapier' }].map(t => (
            <button key={t.id} onClick={() => setPageTab(t.id)} style={{
              ...
              fontSize: 'var(--fs-base, 0.875rem)', fontWeight: pageTab === t.id ? 700 : 500, fontFamily: 'inherit',
          ...
          <>
            ...
            <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
              <input
                ...
                style={{
                  flex: '1 1 220px', padding: '10px 16px', borderRadius: 'var(--r-sm, 10px)',
                  border: '1.5px solid var(--line, #e4e0d8)', fontSize: 'var(--fs-sm, 0.8125rem)',
              ...
              {(search || byName || category !== 'All') && (
                <button
                  ...
                  style={{ padding: '10px 14px', borderRadius: 'var(--r-pill, 999px)', border: '1.5px solid var(--line, #e4e0d8)',
                    background: 'white', fontSize: 'var(--fs-xs, 0.75rem)', fontWeight: 600,
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {loading
                ...
                : filtered.length === 0
                ? (
                  <div style={{ background: 'white', border: '1.5px solid var(--line, #e4e0d8)', borderRadius: 'var(--r-md, 16px)',
                    padding: 40, textAlign: 'center', color: 'var(--taupe, #7a7670)', fontSize: 'var(--fs-base, 0.875rem)' }}>
                ...
                : (() => {
                  ...
                  const header = (label, sub) => (
                    <div key={label} style={{ margin: '6px 0 2px' }}>
                      <div style={{ fontSize: 'var(--fs-2xs, 0.6875rem)', fontWeight: 700, letterSpacing: '.09em',
                        textTransform: 'uppercase', color: '#a39e93' }}>{label}</div>
                      {sub && <div style={{ fontSize: 'var(--fs-xs, 0.75rem)', color: 'var(--taupe, #7a7670)', marginTop: 2 }}>{sub}</div>}
                    </div>
                  );
                  const card = (integration) => (
                    <div key={integration.id} id={`int-${integration.id}`}
                      style={highlightId === integration.id
                        ? { borderRadius: 'var(--r-md, 16px)', boxShadow: '0 0 0 3px var(--honey, #f5c842)', transition: 'box-shadow .3s' } : undefined}>
                      <IntegrationCard
...
function ZapierTab() {
  ...
  const sectionTitle = { fontWeight: 700, fontSize: 'var(--fs-base, 0.875rem)', marginBottom: 12 };
  const card = { background: 'white', border: '1.5px solid var(--line, #e4e0d8)', borderRadius: 'var(--r-md, 16px)', padding: 24, marginBottom: 16 };

  return (
    <div>
      {/* Intro */}
      <div style={{ ...card, background: 'var(--ink, #0a0a0a)', border: 'none' }}>
        <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 'var(--fs-xl, 1.25rem)', fontWeight: 900, color: 'white', marginBottom: 6 }}>
          Connect SwarmReply to 7,000+ apps
        </div>
        <div style={{ fontSize: 'var(--fs-sm, 0.8125rem)', color: 'rgba(255,255,255,.55)', lineHeight: 1.7 }}>
      ...
      <div style={card}>
        ...
        {error && (
          <div style={{ background: 'var(--danger-bg, #fee2e2)', border: '1px solid #fca5a5', borderRadius: 'var(--r-sm, 10px)',
            padding: '10px 14px', fontSize: 'var(--fs-sm, 0.8125rem)', color: 'var(--danger, #c0392b)', marginBottom: 12 }}>{error}</div>
        )}

        {newKey ? (
          <div style={{ background: '#fef9e7', border: '1.5px solid var(--honey, #f5c842)', borderRadius: 'var(--r-md, 16px)', padding: '14px 16px', marginBottom: 4 }}>
            <div style={{ fontSize: 'var(--fs-xs, 0.75rem)', fontWeight: 700, letterSpacing: '.06em', textTransform: 'uppercase', color: 'var(--amber-tx, #92690a)', marginBottom: 8 }}>
              Copy this key now — it won't be shown again
            </div>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              <code style={{ flex: 1, fontFamily: 'monospace', fontSize: 'var(--fs-xs, 0.75rem)', background: 'white',
                border: '1.5px solid var(--line, #e4e0d8)', borderRadius: 'var(--r-xs, 8px)', padding: '8px 12px', color: 'var(--tx, #1a1a18)',
                overflowX: 'auto', whiteSpace: 'nowrap' }}>{newKey}</code>
              <button onClick={copyKey} style={{ padding: '8px 16px', borderRadius: 'var(--r-pill, 999px)',
                border: 'none', background: 'var(--ink, #0a0a0a)', color: 'white', fontSize: 'var(--fs-xs, 0.75rem)',
                fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' }}>
                {copied ? 'Copied ✓' : 'Copy'}
              </button>
            </div>
            <div style={{ fontSize: 'var(--fs-xs, 0.75rem)', color: 'var(--amber-tx, #92690a)', marginTop: 8 }}>
              Paste it into Zapier when the SwarmReply app asks for your API key.
            </div>
          </div>
        ) : keyStatus === null ? (
          <div style={{ color: 'var(--taupe, #7a7670)', fontSize: 'var(--fs-sm, 0.8125rem)' }}>Loading…</div>
        ) : keyStatus.exists ? (
          <>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 12 }}>
              <code style={{ flex: 1, fontFamily: 'monospace', fontSize: 'var(--fs-xs, 0.75rem)', background: 'var(--cream, #f8f7f4)',
                border: '1.5px solid var(--line, #e4e0d8)', borderRadius: 'var(--r-xs, 8px)', padding: '8px 12px', color: 'var(--tx, #1a1a18)' }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              <button onClick={() => generateKey(true)} disabled={busy} style={{ padding: '9px 18px',
                borderRadius: 'var(--r-pill, 999px)', border: '1.5px solid var(--line, #e4e0d8)', background: 'white', fontSize: 'var(--fs-sm, 0.8125rem)',
              ...
              <button onClick={disconnect} disabled={busy} style={{ padding: '9px 18px',
                borderRadius: 'var(--r-pill, 999px)', border: '1.5px solid #fca5a5', background: 'white', fontSize: 'var(--fs-sm, 0.8125rem)',
                fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', color: 'var(--danger, #c0392b)' }}>Disconnect Zapier</button>
            </div>
          </>
        ) : (
          <>
            <div style={{ fontSize: 'var(--fs-sm, 0.8125rem)', color: 'var(--taupe, #7a7670)', lineHeight: 1.7, marginBottom: 14 }}>
            ...
            <button onClick={() => generateKey(false)} disabled={busy} style={{ padding: '11px 24px',
              borderRadius: 'var(--r-pill, 999px)', border: 'none', background: 'var(--ink, #0a0a0a)', color: 'white',
              fontSize: 'var(--fs-sm, 0.8125rem)', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
      ...
      <div style={card}>
        ...
        {[
          ...
          <div key={n} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', marginBottom: 10 }}>
            <div style={{ width: 22, height: 22, borderRadius: 'var(--r-full, 50%)', background: 'var(--honey, #f5c842)', color: 'var(--ink, #0a0a0a)',
              fontSize: 'var(--fs-xs, 0.75rem)', fontWeight: 800, display: 'flex', alignItems: 'center',
              justifyContent: 'center', flexShrink: 0 }}>{n}</div>
            <div style={{ fontSize: 'var(--fs-sm, 0.8125rem)', color: 'var(--tx-3, #3a3a38)', lineHeight: 1.6 }}>{text}</div>
          </div>
        ))}
      </div>
      ...
      <div style={card}>
        ...
        {ZAP_TRIGGERS.map(t => (
          <div key={t.name} style={{ padding: '12px 0', borderBottom: '1px solid var(--cream-2, #f0eeea)' }}>
            <div style={{ fontSize: 'var(--fs-sm, 0.8125rem)', fontWeight: 700, marginBottom: 4 }}>⚡ {t.name}</div>
            <div style={{ fontSize: 'var(--fs-sm, 0.8125rem)', color: 'var(--taupe, #7a7670)', lineHeight: 1.6 }}>{t.desc}</div>
          </div>
        ))}
      </div>
      ...
      <div style={card}>
        ...
        {ZAP_ACTIONS.map(a => (
          <div key={a.name} style={{ padding: '12px 0', borderBottom: '1px solid var(--cream-2, #f0eeea)' }}>
            <div style={{ fontSize: 'var(--fs-sm, 0.8125rem)', fontWeight: 700, marginBottom: 4 }}>▸ {a.name}</div>
            <div style={{ fontSize: 'var(--fs-sm, 0.8125rem)', color: 'var(--taupe, #7a7670)', lineHeight: 1.6 }}>{a.desc}</div>
          </div>
        ))}
      </div>
    </div>

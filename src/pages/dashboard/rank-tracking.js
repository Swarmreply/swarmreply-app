...
function TrendBadge({ trend, delta }) {
  if (!trend || trend === 'stable') return <span style={{ fontSize: '.72rem', color: 'var(--taupe, #7a7670)' }}>—</span>;
  const up = trend === 'up';
  return (
    <span style={{ fontSize: '.72rem', fontWeight: 700, color: up ? 'var(--green, #1a6b45)' : 'var(--danger, #c0392b)', background: up ? 'var(--green-bg, #e8f5ef)' : 'var(--danger-bg, #fee2e2)', padding: '2px 7px', borderRadius: 50 }}>
...
function PositionBadge({ position, inLocalPack, packPosition }) {
  if (!position && !inLocalPack) {
    return <span style={{ fontSize: '.8rem', color: 'var(--taupe, #7a7670)' }}>Not found</span>;
  }
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4, alignItems: 'flex-start' }}>
      {position && (
        <span style={{ fontFamily: "'Playfair Display',serif", fontSize: '1.4rem', fontWeight: 900, color: position <= 3 ? 'var(--green, #1a6b45)' : position <= 10 ? '#f59e0b' : 'var(--taupe, #7a7670)', lineHeight: 1 }}>
          #{position}
        </span>
      )}
      {inLocalPack && (
        <span style={{ fontSize: '.67rem', fontWeight: 700, background: 'var(--green-bg, #e8f5ef)', color: 'var(--green, #1a6b45)', padding: '2px 7px', borderRadius: 50 }}>
...
function MiniChart({ history }) {
  if (!history?.length) return <div style={{ fontSize: '.72rem', color: 'var(--taupe, #7a7670)' }}>No data yet</div>;
  const positions = history.filter(h => h.position).map(h => h.position);
  if (!positions.length) return <div style={{ fontSize: '.72rem', color: 'var(--taupe, #7a7670)' }}>Not ranked</div>;
        ...
        <div className="m-grid-1" style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 16, alignItems: 'start' }}>
          ...
          <div style={{ background: 'white', border: '1.5px solid var(--line, #e4e0d8)', borderRadius: 14, overflow: 'hidden' }}>
            <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--line, #e4e0d8)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              {checking ? (
                <span style={{ fontSize: '.82rem', color: 'var(--amber-tx, #92690a)', fontWeight: 700 }}>↻ Checking…</span>
              ) : lastChecked && nextCheck && new Date(nextCheck) > new Date() ? (
                <div style={{ background: 'var(--cream, #f8f7f4)', border: '1.5px solid var(--line, #e4e0d8)', borderRadius: 8, padding: '5px 12px', textAlign: 'right' }}>
                  <div style={{ fontSize: '.66rem', color: 'var(--taupe, #7a7670)', textTransform: 'uppercase', letterSpacing: '.06em', fontWeight: 700 }}>Next scan</div>
                  <div style={{ fontSize: '.8rem', fontWeight: 600, color: 'var(--ink, #0a0a0a)', marginTop: 1 }}>
                  ...
                  {data.map(kw => (
                    <tr key={kw.keyword} style={{ borderBottom: '1px solid var(--cream, #f8f7f4)' }}>
                      <td style={{ padding: '12px 16px' }}>
                        ...
                        {kw.isAuto && <span style={{ fontSize: '.65rem', color: 'var(--taupe, #7a7670)', background: 'var(--cream-2, #f0eeea)', padding: '1px 6px', borderRadius: 50 }}>auto</span>}
                      </td>
                      <td style={{ padding: '12px 16px' }}>
                      ...
                      <td style={{ padding: '12px 16px' }}>
                        {!kw.isAuto && (
                          <button onClick={() => removeKeyword(kw.keywordId)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--taupe, #7a7670)', fontSize: '.9rem', padding: '2px 4px' }}>✕</button>
                        )}
                      </td>
                    </tr>
                  ))}

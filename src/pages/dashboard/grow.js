...
const TABS = [
  ...
  { id: 'manage',    label: 'Manage Contacts'   },
...
function ManageContactsTab() {
  const [contacts, setContacts] = useState([]);
  const [segments, setSegments] = useState([]);
  const [loading, setLoading]   = useState(true);
  const [q, setQ]               = useState('');
  const [segFilter, setSegFilter]         = useState('all');
  const [statusFilter, setStatusFilter]   = useState('all');   // all | active | opted_out
  const [channelFilter, setChannelFilter] = useState('all');   // all | email | phone
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await axios.get(`${API}/contacts`, { headers: authHeaders() });
      setContacts(res.data.contacts || []);
      setSegments(res.data.segments || []);
    } catch (e) { setContacts([]); }
  ...
  const filtered = contacts.filter(c => {
    if (q && ![c.name, c.email, c.phone].some(v => v && lc(v).includes(lc(q)))) return false;
    if (segFilter !== 'all' && c.segment !== segFilter) return false;
    if (statusFilter === 'opted_out' && !c.opted_out) return false;
    if (statusFilter === 'active' && c.opted_out) return false;
  ...
  const allSel = filtered.length > 0 && filtered.every(c => selected.includes(c.id));
  const toggleSel = (id) => setSelected(s => s.includes(id) ? s.filter(x => x !== id) : [...s, id]);
  const toggleSelAll = () => setSelected(allSel ? [] : filtered.map(c => c.id));
  function flash(m) { setToast(m); setTimeout(() => setToast(''), 2500); }

  function openAdd() { setForm({ name: '', email: '', phone: '', segment: '' }); setFormErr(''); setModal({ mode: 'add' }); }
  function openEdit(c) { setForm({ name: c.name || '', email: c.email || '', phone: c.phone || '', segment: (c.segment && c.segment !== 'all') ? c.segment : '' }); setFormErr(''); setModal({ mode: 'edit', contact: c }); }

  async function saveContact() {
    if (!form.email.trim() && !form.phone.trim()) { setFormErr('Add at least an email or mobile number.'); return; }
    setSaving(true); setFormErr('');
    try {
      if (modal.mode === 'add') { await axios.post(`${API}/contacts`, form, { headers: authHeaders() }); flash('Contact added'); }
      else { await axios.put(`${API}/contacts/${modal.contact.id}`, form, { headers: authHeaders() }); flash('Contact updated'); }
      setModal(null); await load();
    } catch (e) { setFormErr(e.response?.data?.error || 'Could not save contact.'); }
    finally { setSaving(false); }
  }

  async function removeContact(c) {
    if (!window.confirm(`Delete ${c.name || c.email || c.phone || 'this contact'}? This removes them from your contact list.`)) return;
    setBusyId(c.id);
    try { await axios.delete(`${API}/contacts/${c.id}`, { headers: authHeaders() }); setSelected(s => s.filter(x => x !== c.id)); await load(); flash('Contact deleted'); }
    catch (e) { alert(e.response?.data?.error || 'Could not delete.'); }
    finally { setBusyId(null); }
  }

  async function toggleOpt(c) {
    setBusyId(c.id);
    try { await axios.post(`${API}/contacts/${c.id}/opt-out`, { opted_out: !c.opted_out }, { headers: authHeaders() }); await load(); flash(c.opted_out ? 'Opted back in' : 'Opted out'); }
    catch (e) { alert(e.response?.data?.error || 'Could not update.'); }
    finally { setBusyId(null); }
  }

  async function bulk(action) {
    if (selected.length === 0) return;
    if (action === 'delete' && !window.confirm(`Delete ${selected.length} contact${selected.length === 1 ? '' : 's'}? This cannot be undone.`)) return;
    setBulkBusy(true);
    try {
      await axios.post(`${API}/contacts/bulk`, { contactIds: selected, action }, { headers: authHeaders() });
      setSelected([]); await load();
      flash(action === 'delete' ? 'Contacts deleted' : action === 'opt_out' ? 'Contacts opted out' : 'Contacts opted in');
    } catch (e) { alert(e.response?.data?.error || 'Bulk action failed.'); }
    finally { setBulkBusy(false); }
  }

  const inp      = { width: '100%', padding: '10px 12px', border: '1.5px solid #e4e0d8', borderRadius: 9, fontSize: '.85rem', fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box' };
  const selSt    = { padding: '8px 10px', border: '1.5px solid #e4e0d8', borderRadius: 9, fontSize: '.8rem', fontFamily: 'inherit', background: 'white', color: '#0a0a0a' };
  const darkBtn  = { padding: '8px 16px', borderRadius: 50, background: '#0a0a0a', color: 'white', border: 'none', cursor: 'pointer', fontSize: '.82rem', fontWeight: 700, fontFamily: 'inherit' };
  const ghostBtn = { padding: '8px 14px', borderRadius: 50, background: 'white', color: '#4a4a48', border: '1.5px solid #e4e0d8', cursor: 'pointer', fontSize: '.82rem', fontWeight: 600, fontFamily: 'inherit' };
  const actBtn   = { padding: '4px 9px', borderRadius: 7, border: '1.5px solid #e4e0d8', background: 'white', cursor: 'pointer', fontSize: '.72rem', fontWeight: 600, fontFamily: 'inherit', whiteSpace: 'nowrap', color: '#4a4a48' };

  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12, marginBottom: 16 }}>
        <div>
          <div style={{ fontWeight: 700, fontSize: '1.05rem' }}>Manage contacts</div>
          <div style={{ fontSize: '.82rem', color: '#7a7670' }}>Search, edit, and organize everyone in your contact list.</div>
        </div>
        <button onClick={openAdd} style={darkBtn}>+ Add contact</button>
      </div>
      ...
      <div className="m-grid-2" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 16 }}>
        ...
        <StatCard label="With mobile" value={stats.withPhone.toLocaleString()} />
        <StatCard label="Opted out" value={stats.optedOut.toLocaleString()} />
      </div>

      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 14, alignItems: 'center' }}>
        <input value={q} onChange={e => setQ(e.target.value)} placeholder="Search name, email, or phone…" style={{ ...inp, flex: 1, minWidth: 200, maxWidth: 360 }} />
        <select value={segFilter} onChange={e => setSegFilter(e.target.value)} style={selSt}>
          {segments.map(s => <option key={s.id} value={s.id}>{s.id === 'all' ? 'All segments' : s.name} ({s.count})</option>)}
        </select>
        ...
        <select value={channelFilter} onChange={e => setChannelFilter(e.target.value)} style={selSt}>
          <option value="all">Any channel</option>
          <option value="email">Has email</option>
          <option value="phone">Has mobile</option>
        </select>
      </div>

      {selected.length > 0 && (
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap', background: '#0a0a0a', color: 'white', borderRadius: 12, padding: '10px 16px', marginBottom: 14 }}>
          <span style={{ fontSize: '.82rem', fontWeight: 700 }}>{selected.length} selected</span>
          <div style={{ flex: 1 }} />
          <button onClick={() => bulk('opt_out')} disabled={bulkBusy} style={{ ...ghostBtn, background: 'transparent', color: 'white', borderColor: 'rgba(255,255,255,.3)' }}>Opt out</button>
          <button onClick={() => bulk('opt_in')} disabled={bulkBusy} style={{ ...ghostBtn, background: 'transparent', color: 'white', borderColor: 'rgba(255,255,255,.3)' }}>Opt in</button>
          <button onClick={() => bulk('delete')} disabled={bulkBusy} style={{ ...ghostBtn, background: 'transparent', color: '#ff9b8a', borderColor: 'rgba(255,155,138,.4)' }}>Delete</button>
          <button onClick={() => setSelected([])} style={{ ...ghostBtn, background: 'transparent', color: 'rgba(255,255,255,.7)', border: 'none' }}>Clear</button>
        </div>
      )}

      <Card style={{ overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 20px', borderBottom: '1px solid #e4e0d8', background: '#fafaf9' }}>
          <div onClick={toggleSelAll} style={{ width: 18, height: 18, borderRadius: 5, border: '2px solid', borderColor: allSel ? '#0a0a0a' : '#c8c4bc', background: allSel ? '#0a0a0a' : 'white', cursor: 'pointer', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {allSel && <span style={{ color: 'white', fontSize: '.6rem', fontWeight: 900 }}>✓</span>}
          </div>
          <span style={{ fontSize: '.78rem', color: '#7a7670', fontWeight: 600 }}>{filtered.length} contact{filtered.length === 1 ? '' : 's'}{filtered.length !== contacts.length ? ` of ${contacts.length}` : ''}</span>
        </div>

        {loading ? (
          <div style={{ padding: 40, textAlign: 'center', color: '#7a7670', fontSize: '.85rem' }}>Loading contacts…</div>
        ) : filtered.length === 0 ? (
          <div style={{ padding: 16 }}>
            <EmptyState compact title={contacts.length === 0 ? 'No contacts yet' : 'No matches'} description={contacts.length === 0 ? 'Add a contact above, or import a CSV on the Import tab.' : 'Try a different search or filter.'} />
          </div>
        ) : (
          <div style={{ maxHeight: 'calc(100vh - 420px)', overflowY: 'auto' }}>
            {filtered.map(c => {
              ...
              const isOut = !!c.opted_out;
              return (
                <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 20px', borderBottom: '1px solid #f8f7f4', background: isSel ? '#fafaf9' : 'white' }}>
                  <div onClick={() => toggleSel(c.id)} style={{ width: 18, height: 18, borderRadius: 5, border: '2px solid', borderColor: isSel ? '#0a0a0a' : '#c8c4bc', background: isSel ? '#0a0a0a' : 'white', cursor: 'pointer', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {isSel && <span style={{ color: 'white', fontSize: '.6rem', fontWeight: 900 }}>✓</span>}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 600, fontSize: '.84rem', color: '#0a0a0a', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{c.name || '(no name)'}</div>
                    <div style={{ fontSize: '.75rem', color: '#7a7670', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{c.email || '—'}{c.phone ? ' · ' + c.phone : ''}</div>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                    {isOut && <span style={{ fontSize: '.64rem', color: '#c0392b', background: '#fee2e2', padding: '2px 7px', borderRadius: 50, fontWeight: 700 }}>Opted out</span>}
                    {c.segment && c.segment !== 'all' && <span style={{ fontSize: '.64rem', color: '#4a4a48', background: '#f0eeea', padding: '2px 7px', borderRadius: 50, textTransform: 'capitalize' }}>{c.segment}</span>}
                    <button onClick={() => openEdit(c)} style={actBtn}>Edit</button>
                    <button onClick={() => toggleOpt(c)} disabled={busyId === c.id} style={{ ...actBtn, color: isOut ? '#1a6b45' : '#92690a' }}>{busyId === c.id ? '…' : (isOut ? 'Opt in' : 'Opt out')}</button>
                    <button onClick={() => removeContact(c)} disabled={busyId === c.id} style={{ ...actBtn, color: '#c0392b' }}>Delete</button>
                  </div>
                </div>
              );
            })}
      ...
      {toast && <div style={{ position: 'fixed', bottom: 24, left: '50%', transform: 'translateX(-50%)', background: '#0a0a0a', color: 'white', padding: '10px 20px', borderRadius: 50, fontSize: '.82rem', fontWeight: 600, zIndex: 200, boxShadow: '0 8px 28px rgba(0,0,0,.2)' }}>{toast}</div>}

      {modal && (
        <div onClick={() => !saving && setModal(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: 16 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'white', borderRadius: 16, padding: 24, width: '100%', maxWidth: 420, boxShadow: '0 20px 60px rgba(0,0,0,.25)' }}>
            <div style={{ fontWeight: 700, fontSize: '1.05rem', marginBottom: 16 }}>{modal.mode === 'add' ? 'Add contact' : 'Edit contact'}</div>
            <label style={{ display: 'block', fontSize: '.75rem', fontWeight: 600, color: '#7a7670', marginBottom: 4 }}>Name</label>
            <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Jane Doe" style={{ ...inp, marginBottom: 12 }} />
            <label style={{ display: 'block', fontSize: '.75rem', fontWeight: 600, color: '#7a7670', marginBottom: 4 }}>Email</label>
            <input value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} placeholder="jane@example.com" style={{ ...inp, marginBottom: 12 }} />
            <label style={{ display: 'block', fontSize: '.75rem', fontWeight: 600, color: '#7a7670', marginBottom: 4 }}>Mobile phone</label>
            <input value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} placeholder="+1 555 123 4567" style={{ ...inp, marginBottom: 12 }} />
            <label style={{ display: 'block', fontSize: '.75rem', fontWeight: 600, color: '#7a7670', marginBottom: 4 }}>Segment (optional)</label>
            <input value={form.segment} onChange={e => setForm(f => ({ ...f, segment: e.target.value }))} placeholder="e.g. vip" list="manage-seg-options" style={{ ...inp, marginBottom: 6 }} />
            <datalist id="manage-seg-options">{segments.filter(s => s.id !== 'all').map(s => <option key={s.id} value={s.id} />)}</datalist>
            <div style={{ fontSize: '.72rem', color: '#a8a39a', marginBottom: 14 }}>Add at least an email or a mobile number.</div>
            {formErr && <div style={{ color: '#c0392b', fontSize: '.8rem', marginBottom: 12 }}>{formErr}</div>}
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={saveContact} disabled={saving} style={{ ...darkBtn, opacity: saving ? .6 : 1 }}>{saving ? 'Saving…' : (modal.mode === 'add' ? 'Add contact' : 'Save changes')}</button>
              <button onClick={() => setModal(null)} disabled={saving} style={ghostBtn}>Cancel</button>
            </div>
          </div>
        </div>
      )}
...
export default function Grow() {
  ...
  useEffect(() => {
    ...
    if (['requests', 'templates', 'bulk', 'manage', 'import'].includes(t)) setTab(t);
  }, [router.query?.tab]);
